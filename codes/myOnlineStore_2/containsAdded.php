			...
			if($is_staff == 'N' && $order_id != ''){ //IS NOT STAFF
				if(isset($_POST['submit'])){
					...
					if(empty($data_missing)){
						...
						$query = "INSERT INTO Contains (order_id, quantity, product_id) VALUES (?, ?, ?)";
						...
						mysqli_stmt_bind_param($stmt, "iii", $order_id, $quantity, $product_id);
						mysqli_stmt_execute($stmt);
						
						$affected_rows = mysqli_stmt_affected_rows($stmt);
						
						if($affected_rows == 1){
							echo "Order/Cart updated with new product.";
			...
			if($is_staff == 'Y'){  //IS STAFF
				if(isset($_POST['submit'])){
					...
					if(empty($data_missing)){
						...
						mysqli_stmt_bind_param($stmt, "iii", $orderid, $quantity, $product_id);
						mysqli_stmt_execute($stmt);

<html>
	...
	<body>
		...
		<?php
			...
			if($is_staff == 'N' && $order_id != ''){ //IS NOT STAFF
				if(isset($_POST['submit'])){
					...
					//if(empty($data_missing)){
						...
						$query = "DELETE FROM OrderMade WHERE order_id = ?";
						...
						if($affected_rows == 1){
							echo "Order/Cart successfully deleted.";
			...
			if($is_staff == 'Y'){  //IS STAFF
				if(isset($_POST['submit'])){
					...
					if(empty($data_missing)){
						require_once('../mysqli_connect.php');
						$query = "DELETE FROM OrderMade WHERE id = $orderid";
		...
		?>
		<p>Back to order/cart menu <a href="http://localhost/codes/myonlinestore_2/orderMenu.php"> click</a></p> 
	</body>
</html>
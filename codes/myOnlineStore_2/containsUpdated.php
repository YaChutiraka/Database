			...
			if($is_staff == 'N' && $order_id != ''){ //IS NOT STAFF
				if(isset($_POST['submit'])){
					...
					if(empty($data_missing)){
						...
						$query = "UPDATE Contains SET quantity = ? WHERE order_id = ? AND product_id = ?";
						...
						if($affected_rows == 1){
							echo "Order/Cart updated with new product quantity.";
			...
			if($is_staff == 'Y'){  //IS STAFF
				if(isset($_POST['submit'])){
					...
					if(empty($data_missing)){
						...
						mysqli_stmt_bind_param($stmt, "iii", $quantity, $orderid, $product_id);
						mysqli_stmt_execute($stmt);
